...
Enable session replay protection, so that a session cookie cannot be replayed if the user logs out
...
(NOT FOR PRODUCTION USE) Enable session timeout based on inactivity and maximum duration. Inactivity timeout is always enforced when enabled. Maximum duration enforcement additionally requires SESSION_REPLAY_PROTECTION_ENABLED=true.
