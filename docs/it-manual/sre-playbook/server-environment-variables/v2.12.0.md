...
Overrides the default configuration for the content security policy. If set to true, the browser reports content security policy violations but does not enforce the policy. If set to false, the browser enforces the policy.
...
(NOT FOR PRODUCTION USE) When enabled, admins will be able to select many applications for status updates
