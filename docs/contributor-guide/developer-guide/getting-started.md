...
Github offers an interface for searching for the code and you can choose to search through the entire [CiviForm organization](https://github.com/search?q=org%3Aciviform+&type=code) (which includes repositories for deployment specific code, these docs, etc.), or specifically [CiviForm application code](https://github.com/search?q=repo%3Aciviform%2Fciviform%20&type=code). 

There is also an [extension](https://docs.sourcegraph.com/integration/browser_extension) called SourceGraph that can be integrated with Github, and provides a separate [search interface](https://sourcegraph.com/search?q=context:global+repo:%5Egithub%5C.com/civiform/civiform%24++OR+repo:%5Egithub%5C.com/civiform/cloud-deploy-infra%24+&patternType=standard&sm=0&groupBy=repo) as well.

It can be helpful to add a [search engine extension](chrome://settings/searchEngines) to help make code searching easier. Here are the URLs:

* Github CiviForm Search [URL](https://github.com/search?q=org%3Aciviform%20%s&type=code)
* Souregraph Civiform Search [URL](https://sourcegraph.com/search?q=%s+repo%3Aciviform%2Fciviform%24)
