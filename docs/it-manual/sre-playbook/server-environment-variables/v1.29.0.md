...
The API key used to interact with LoginRadius. **Required**.

- Type: string

#### LOGIN_RADIUS_METADATA_URI

The base URL to construct SAML endpoints, based on the SAML2 spec. **Required**.

- Type: string

#### LOGIN_RADIUS_SAML_APP_NAME

The name for the app, based on the SAML2 spec. **Required**.
...
A URL that returns a JSON listing of OIDC (OpenID Connect) data associated with a given auth provider. **Required**.
...
A URL that returns a JSON listing of OIDC (OpenID Connect) data associated with the IDCS auth provider. **Required**.
