...
Logo for the civic entity used in the footer. If not set, falls back to CIVIC_ENTITY_SMALL_LOGO_URL.
...
Whether the WHITELABEL_CIVIC_ENTITY_SHORT_NAME should be hidden in the CiviForm header. This may be desired if the government name is included in the logo. The logo is hidden on smaller screens, so this will only hide the name if the logo is showing.
...
How long in milliseconds a database connection can be borrowed from the pool before a warning log is generated indicating a potential connection leak. Minimum value is 2000.
...
Enables new visibility/eligibility condition editing UI and expanded logic capabilities for admin.
...
(NOT FOR PRODUCTION USE) Enables optional scores on multi-option question answer options and sums all scores on pdf downloads, csv exports, and JSON exports of submitted applications for admins and TIs on programs that opt in.
